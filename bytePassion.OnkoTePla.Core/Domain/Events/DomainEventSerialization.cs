...
namespace bytePassion.OnkoTePla.Core.Domain.Events
{
	public static class DomainEventSerialization
	{
		private static string SerializeAppointmentAdded (AppointmentAdded addedEvent)
		{
			...
			sb.Append(addedEvent.CreateAppointmentData.PatientId);      sb.Append("|");
			sb.Append(addedEvent.CreateAppointmentData.Description);    sb.Append("|");
			sb.Append(addedEvent.CreateAppointmentData.Day);            sb.Append("|");
			sb.Append(addedEvent.CreateAppointmentData.StartTime);      sb.Append("|");
			sb.Append(addedEvent.CreateAppointmentData.EndTime);        sb.Append("|");
			sb.Append(addedEvent.CreateAppointmentData.TherapyPlaceId); sb.Append("|");
			sb.Append(addedEvent.CreateAppointmentData.AppointmentId);  sb.Append("|");
		...
		private static AppointmentAdded DeserializeAppointmentAdded(string s)
		{
			...
			var creationDataDay            = Date.Parse(eventParts[6]);
			var creationDataStartTime      = Time.Parse(eventParts[7]);
			var creationDataEndTime        = Time.Parse(eventParts[8]);
			var creationDataTherapyPlaceId = Guid.Parse(eventParts[9]);
			var creationDataAppointmentId  = Guid.Parse(eventParts[10]);

			var creationData = new CreateAppointmentData(creationDataPatientId, 
			...
			return new AppointmentAdded(aggregateId, aggregateVersion, userId, timeStamp, actionTag, creationData);
		}
